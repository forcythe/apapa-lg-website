...
module.exports = {
  async vote(ctx) {
    try {
      ...
      // DEBUG: confirm hashing is working before DB write
      strapi.log.info(`Raw NIN length: ${nin.trim().length}`);
      strapi.log.info(`Hashed NIN: ${hashedNin}`);
      ...
      // Create vote with hashed NIN
      const createdVote = await strapi.db.query(UID_VOTE).create({
        data: {
          poll: poll.id,
          optionId: Number(optionId),
          nin: hashedNin,
      ...
      strapi.log.info(`Saved vote nin field: ${createdVote.nin}`);
